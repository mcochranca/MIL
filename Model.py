...
class Model(nn.Module):
    def __init__(self, config, mode):
        ...
        self.config = config
        self.orig_d_l, self.orig_d_v = 768, 867
        self.d_l, self.d_v = 30, 30
        self.num_heads = 5
        self.layers = 5
        ...
        self.attn_mask = True
        self.seq_len_l = config.sentence_max_size
    ...
    def forward(self, x_l, mask, x_v):
        """
        text and vision should have dimension [batch_size, seq_len, n_features]
        """
        x_l = x_l.view(-1, self.seq_len_l)
        x_l = self.bert(input_ids=x_l, attention_mask=mask).last_hidden_state
        x_l = F.dropout(x_l.transpose(1, 2), p=self.embed_dropout, training=self.training)
        x_v = x_v.transpose(1, 2)  # out = bt * orig_d_v * seq_len

        # Project the textual/visual features
        proj_x_l = x_l if self.orig_d_l == self.d_l else self.proj_l(x_l)  # out = bt * d_l * seq_len
        proj_x_v = x_v if self.orig_d_v == self.d_v else self.proj_v(x_v)
        proj_x_v = proj_x_v.permute(2, 0, 1)
        proj_x_l = proj_x_l.permute(2, 0, 1)  # out = seq_len * bt * d_l
